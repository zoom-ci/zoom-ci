    ...
    <div>
        ...
        <el-card shadow="never" v-else>
            ...
            <el-table
                ...
                <el-table-column :label="$t('operate')" width="420" align="right">
                    <template slot-scope="scope">
                        <el-button
                        ...
                        <el-button
                        v-if="$root.CheckPriv($root.Priv.PROJECT_COPY)"
                        icon="el-icon-document-copy"
                        type="text"
                        @click="doCopyDialogHandler(scope.row)">{{ $t('copy') }}</el-button>
                        <el-button
...
export default {
    ...
    methods: {
        ...
        doCopyDialogHandler(row) {
            this.$root.ConfirmCopy(() => {
                copyProjectApi({id: row.id}).then(res => {
                    this.$root.MessageSuccess()
                    this.$root.PageReset()
                    this.loadTableData()
                })
