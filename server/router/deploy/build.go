...
import (
	...
	buiTask "github.com/zoom-ci/zoom-ci/server/build"
...
func BuildStart(c *gin.Context) {
	...
	// build
	workSpace := gostring.JoinStrings(zoom.App.LocalTmpSpace, "/", gostring.Int2Str(build.ApplyId))
	packFile := gostring.JoinStrings(zoom.App.LocalTarSpace, "/", gostring.Int2Str(build.ApplyId), ".tgz")
	...
	bui, err := buiTask.NewBuild(repo, workSpace, zoom.App.LocalTmpSpace, packFile, p.BuildScript)
	if err != nil {
		render.AppError(c, err.Error())
		return
	}
