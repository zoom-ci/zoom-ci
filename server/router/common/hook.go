...
import (
	...
	"github.com/zoom-ci/zoom-ci/util/gofile"
...
func hookCommandTaskRun(s string, applyId int) {
	scriptFile := gostring.JoinStrings(zoom.App.LocalTmpSpace, "/", gostring.StrRandom(24), ".sh")
	if err := gofile.CreateFile(scriptFile, []byte(s), 0744); err != nil {
		zoom.App.Logger.Error("hook script file create failed, err[%s], apply_id[%d]", err.Error(), applyId)
		return
	}
	cmds := []string{
	...
	if err := task.GetError(); err != nil {
		zoom.App.Logger.Error("hook script run failed, err[%s], output[%s], apply_id[%d]", err.Error(), gostring.JsonEncode(task.Result()), applyId)
	}
}
