...
import (
	...
	"github.com/zoom-ci/zoom-ci/util/goaes"
...
func (login *Login) ValidateToken() error {
	...
	tokenValBytes, err := goaes.Decrypt(zoom.App.CipherKey, authTokenBytes)
	if err != nil {
...
func (login *Login) createToken() error {
	...
	tokenBytes, err = goaes.Encrypt(zoom.App.CipherKey, []byte(loginRaw))
	if err != nil {
		return err
	}
